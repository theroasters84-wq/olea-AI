...
def paragogi_syntaghs(ktima_id):
    ...
    try:
        ...
        prompt = (
            ...
            f"1α. ΒΑΣΙΚΗ ΠΡΟΛΗΨΗ: Την Άνοιξη (Μάρτιο/Απρίλιο) και το Φθινόπωρο, ο προληπτικός ψεκασμός με Χαλκό ή Μυκητοκτόνα (π.χ. για κυκλοκόνιο) είναι απαραίτητος. Αν δεν υπάρχει τέτοια καταγραφή στο ιστορικό, συμπερίλαβέ τον στις άμεσες προτεραιότητες, ΕΚΤΟΣ αν το δέντρο βρίσκεται στο στάδιο της πλήρους άνθισης ή πολύ κοντά σε αυτήν, οπότε και ΑΠΑΓΟΡΕΥΕΤΑΙ ΑΥΣΤΗΡΑ ΚΑΘΕ ΨΕΚΑΣΜΟΣ για να μην καούν τα άνθη.\n"
            ...
            f"1στ. ΥΠΟΛΟΓΙΣΜΟΣ ΔΟΣΟΛΟΓΙΑΣ & ΙΣΤΟΡΙΚΟ: Υπολόγισε την προτεινόμενη συνολική δοσολογία με βάση τον Αριθμό Δέντρων και την Ηλικία. ΑΝ από το 'ΙΣΤΟΡΙΚΟ ΠΡΟΒΛΗΜΑΤΩΝ, ΦΩΤΟΓΡΑΦΙΩΝ & ΕΥΡΗΜΑΤΩΝ ΓΡΑΜΜΑΤΕΑ' προκύπτει πρόσφατη αναγνώριση ασθένειας (π.χ. Κυκλοκόνιο, μύκητες) ή συγκεκριμένο στάδιο, ΛΑΒΕ ΤΟ ΣΟΒΑΡΑ ΥΠΟΨΗ για τη συνταγή σου, προσαρμόζοντας τα φάρμακα ή τις δόσεις. Παρουσίασε αναλυτικά τους υπολογισμούς στο κείμενο.\n"
            f"1ζ. ΜΕΣΟ ΨΕΚΑΣΜΟΥ: Αν προτείνεις ψεκασμό, ΡΩΤΑ ΞΕΚΑΘΑΡΑ με ποιο μέσο θα γίνει η εφαρμογή (τουρμπίνα, μπεκ/λάστιχο, ψεκαστήρας πλάτης, drone) ώστε στην επόμενη συνομιλία σας να προσαρμόσεις με ακρίβεια τον όγκο του ψεκαστικού υγρού.\n"
            f"1η. ΣΕΙΡΑ ΑΝΑΜΕΙΞΗΣ: Αν προτείνεις ψεκασμό με μίγμα 2 ή παραπάνω σκευασμάτων (π.χ. χαλκός + ιχνοστοιχεία + κολλητικό), ΠΕΡΙΓΡΑΨΕ ΞΕΚΑΘΑΡΑ μέσα στο 'keimeno_syntaghs' την επιστημονικά σωστή σειρά προσθήκης τους στο βυτίο (π.χ. νερό ως τη μέση, μετά σκόνες WP/WG, μετά υγρά SC/EC, τέλος διαβρέκτες/κολλητικά) για να μην κόψει το φάρμακο.\n"
            ...
            f"6. ΕΠΙΣΤΗΜΟΝΙΚΗ ΟΡΘΟΤΗΤΑ: Έχεις ζωντανή πρόσβαση στο διαδίκτυο. Βασίσου στις πιο σύγχρονες γεωπονικές έρευνες και μελέτες για την καλλιέργεια της ελιάς. Πρότεινε την επιστημονικά βέλτιστη λύση, ακόμα κι αν αποκλίνει από 'παραδοσιακές' πρακτικές.\n"
...
def refine_syntagh(ktima_id):
    ...
    try:
        ...
        prompt = (
            ...
            f"ΟΔΗΓΙΑ: Αναπροσάρμοσε την προηγούμενη συνταγή σου με βάση το σχόλιο του αγρότη. Λάβε υπόψη τις ΠΟΙΚΙΛΙΕΣ, τον ΑΡΙΘΜΟ και την ΗΛΙΚΙΑ των δέντρων, καθώς και τα ΠΕΡΣΙΝΑ ΠΡΟΒΛΗΜΑΤΑ (π.χ. δάκος) για να αυξομειώσεις τις δόσεις. Αν ο αγρότης απάντησε με ποιο ΜΕΣΟ ΨΕΚΑΣΜΟΥ θα ψεκάσει, ΠΡΟΣΑΡΜΟΣΕ τον όγκο του υγρού. Αν ο αγρότης ζητήσει να αναμείξει στο βυτίο ουσίες που είναι ασύμβατες (π.χ. Χαλκός με Αμινοξέα), ΑΠΑΓΟΡΕΥΣΕ το έντονα στο κείμενο. ΣΥΓΧΩΝΕΥΣΗ ΨΕΚΑΣΜΩΝ (TANK MIX): Αν η αναθεωρημένη συνταγή απαιτεί πολλαπλά σκευάσματα, συγχώνευσέ τα σε 1 εργασία εφόσον είναι συμβατά (π.χ. Κάλιο + Αμινοξέα) για να ραντίσει μόνο 1 φορά. Αν βάλεις εργασίες με αναμονή, βάλε το tag '[ΧΡΟΝΟΜΕΤΡΟ:YYYY-MM-DD]'. Αν η συνταγή περιλαμβάνει μίγμα, ΕΞΗΓΗΣΕ τη σωστή σειρά ανάμειξης. Φρόντισε κάθε νέα εργασία να έχει ξεκάθαρο χρονικό περιθώριο. ΜΗΝ ρωτάς για τον καιρό ή για προβλέψεις.\n"
            f"ΒΙΟΛΟΓΙΚΗ ΓΕΩΡΓΙΑ: Αν στο προφίλ αναφέρεται 'Βιολογική', ΑΠΑΓΟΡΕΥΕΤΑΙ ΑΥΣΤΗΡΑ η χρήση χημικών ζιζανιοκτόνων ή φαρμάκων.\n"
            f"ΕΠΙΣΤΗΜΟΝΙΚΗ ΟΡΘΟΤΗΤΑ: Διασταύρωσε τη λύση σου με σύγχρονες μελέτες (μέσω internet search) ώστε να είναι η πιο άρτια γεωπονικά.\n"
            f"Επίστρεψε ΑΥΣΤΗΡΑ ΚΑΙ ΜΟΝΟ ένα έγκυρο JSON με αυτή την ακριβή δομή: "
