...
def refine_syntagh(ktima_id):
    ...
    try:
        ...
        prompt = (
            ...
            f"ΟΔΗΓΙΑ: Αναπροσάρμοσε την προηγούμενη συνταγή σου με βάση το σχόλιο του αγρότη. Λάβε υπόψη τις ΠΟΙΚΙΛΙΕΣ, τον ΑΡΙΘΜΟ και την ΗΛΙΚΙΑ των δέντρων, καθώς και τα ΠΕΡΣΙΝΑ ΠΡΟΒΛΗΜΑΤΑ (π.χ. δάκος) για να αυξομειώσεις τις δόσεις. Αναζήτησε στο ίντερνετ την ιδανική δοσολογία φαρμάκου και όγκου νερού ΕΙΔΙΚΑ για την ηλικία των δέντρων, ώστε να μην καούν τα νεαρά δέντρα ή να μην υποδοσολογηθούν τα γηραιά. Αν ο αγρότης απάντησε με ποιο ΜΕΣΟ ΨΕΚΑΣΜΟΥ θα ψεκάσει, ΠΡΟΣΑΡΜΟΣΕ τον όγκο του υγρού. Αν ο αγρότης ζητήσει να αναμείξει στο βυτίο ουσίες που είναι ασύμβατες (π.χ. Χαλκός με Αμινοξέα), ΑΠΑΓΟΡΕΥΣΕ το έντονα στο κείμενο. ΣΥΓΧΩΝΕΥΣΗ ΨΕΚΑΣΜΩΝ (TANK MIX): Αν η αναθεωρημένη συνταγή απαιτεί πολλαπλά σκευάσματα, συγχώνευσέ τα σε 1 εργασία εφόσον είναι συμβατά (π.χ. Κάλιο + Αμινοξέα) για να ραντίσει μόνο 1 φορά. Αν βάλεις εργασίες με αναμονή, βάλε το tag '[ΧΡΟΝΟΜΕΤΡΟ:YYYY-MM-DD]'. Αν η συνταγή περιλαμβάνει μίγμα, ΕΞΗΓΗΣΕ τη σωστή σειρά ανάμειξης. Φρόντισε κάθε νέα εργασία να έχει ξεκάθαρο χρονικό περιθώριο. ΜΗΝ ρωτάς για τον καιρό ή για προβλέψεις.\n"
