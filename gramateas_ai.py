...
def ai_secretary():
    ...
    try:
        ...
        if valid_images:
            text += f"\n[ΣΥΣΤΗΜΙΚΗ ΕΙΔΟΠΟΙΗΣΗ: Ο χρήστης μόλις επισύναψε {len(valid_images)} αρχείο(α)/φωτογραφία(ες). ΔΕΣ ΤΑ ΠΡΟΣΕΚΤΙΚΑ και δώσε του μια ανάλυση/διάγνωση για το τι δείχνουν!]"
