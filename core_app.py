...
def inject_helpers():
    def get_secretary_greeting(user):
        if not getattr(user, 'is_authenticated', False):
            return "Γεια σας! Μπορείτε να μου πείτε τι εργασίες κάνατε."
            
        now = datetime.now()
        is_evening = now.hour >= 19
        target_date = (now + timedelta(days=1)).date() if is_evening else now.date()
        
        tasks_found = []
        reminders_found = []
        future_tasks = []
        
        for k in user.ktimata:
            if getattr(k, 'is_active', False):
                for e in k.ergasies:
                    if not e.archived and getattr(e, 'katastasi', '') == 'Εκκρεμεί' and e.imerominia:
                        # ΝΕΟ: Αγνοούμε τις πολύ παλιές εκκρεμότητες από την πρωινή ενημέρωση
                        days_overdue = (now.date() - e.imerominia.date()).days
                        if days_overdue > 30:
                            continue

                        if e.imerominia.date() <= target_date:
                            if '[AI ΥΠΕΝΘΥΜΙΣΗ]' in e.eidos_ergasias:
                                msg = e.eidos_ergasias.replace('[AI ΥΠΕΝΘΥΜΙΣΗ]', '').strip()
                                reminders_found.append(f"Μου είχατε ζητήσει να σας θυμίσω: <strong>{msg}</strong> στο <em>{k.onoma_ktimatos}</em>.")
                            else:
                                if e.imerominia.date() < now.date():
                                    tasks_found.append(f"<strong class='text-danger'>{e.eidos_ergasias} στο {k.onoma_ktimatos} (Καθυστερημένη)</strong>")
                                else:
                                    tasks_found.append(f"<strong>{e.eidos_ergasias}</strong> στο <em>{k.onoma_ktimatos}</em>")
                        elif e.imerominia.date() > target_date:
                            if '[AI ΥΠΕΝΘΥΜΙΣΗ]' not in e.eidos_ergasias:
                                future_tasks.append((e.imerominia.date(), f"<strong>{e.eidos_ergasias}</strong> στο <em>{k.onoma_ktimatos}</em>"))

        time_word = "αύριο" if is_evening else "σήμερα"
        greeting = "Γεια σας! 👋<br>"
        
        if reminders_found:
            greeting += "<br>💡 <strong>Μήνυμα από το παρελθόν:</strong><br>"
            greeting += "<ul class='mb-2 ps-3 text-primary' style='list-style-type: none;'>" + "".join([f"<li><i class='fas fa-thumbtack text-warning'></i> {r}</li>" for r in reminders_found]) + "</ul>"
            greeting += "Να το βάλουμε στο πρόγραμμα να γίνει τώρα;<br><br>"
        
        if tasks_found:
            greeting += f"Για <strong>{time_word}</strong> έχουμε προγραμματισμένα τα εξής:<br>"
