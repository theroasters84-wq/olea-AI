        ...
        # Fallback to original time/humidity logic if stage is not critical (e.g., 'Άγνωστο', 'Λήθαργος')
            ...
            if days_since_copper is not None:
                if days_since_copper <= 25:
                    ...
                    protaseis.append(f"⚠️ Λήξη Προστασίας: Έχουν περάσει {days_since_copper} μέρες από τον τελευταίο ψεκασμό χαλκού. Απαιτείται επαναληπτικός ψεκασμός. ΠΡΟΣΟΧΗ: Λόγω άνοιξης, επιλέξτε αυστηρά ήπιο χαλκό (π.χ. υδροξείδιο) ή μυκητοκτόνο για να μην καεί η νέα βλάστηση.")
            else:
