...
def evaluate_overdue_tasks(ktima):
    ...
    prompt = (
        ...
        f"Αν το ιδανικό χρονικό παράθυρο ή το στάδιο έχει περάσει (π.χ. ψεκασμός που έπρεπε να γίνει πριν την άνθιση αλλά τώρα το δέντρο ανθίζει, ή χειμερινή εργασία ενώ τώρα έχει ζέστη), η εργασία πρέπει να ΑΚΥΡΩΘΕΙ. "
...
def syghronismos_ai_ktimatos(ktima):
    ...
    prompt = (
        ...
        + (f"ΟΔΗΓΙΑ ΚΡΙΣΙΜΗ: Στο Ημερολόγιο υπάρχουν εκκρεμείς εργασίες που είχαν προγραμματιστεί για ΣΗΜΕΡΑ ή ΠΑΛΑΙΟΤΕΡΑ ({overdue_str}). Ξεκίνα την ΕΡΩΤΗΣΗ ρωτώντας τον αγρότη ξεκάθαρα αν τις ολοκλήρωσε ή αν θέλει να μεταφέρει την ημερομηνία τους! " if overdue_str else "Αν ΔΕΝ του έχεις ξαναμιλήσει και δεν έχει εργασίες, κάνε το αρχικό onboarding ('ΕΡΩΤΗΣΗ: Καλώς ήρθατε!...'). Διαφορετικά, αν έχει ήδη απαντήσει, ΜΗΝ κάνεις άλλη ερώτηση, ΠΑΡΑ ΜΟΝΟ αν προέκυψε μια ΝΕΑ απορία. ") +
